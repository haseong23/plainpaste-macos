...
- **화면에서 텍스트 추출** — **⌃⌥⌘O**로 화면 영역을 드래그하면 그 안의 글자를 OCR해 클립보드로 복사. PowerToys의 "Text Extractor"에 대응
- **잠자기 방지** — 메뉴에서 시스템(또는 화면까지) 잠자기를 막음. PowerToys의 "Awake"에 대응
- **화면 색상 추출** — **⌃⌥⌘C**로 스포이드를 띄워 화면의 색을 집으면 HEX(#RRGGBB)를 클립보드로 복사, 최근 색상 히스토리 제공. PowerToys의 "Color Picker"에 대응
- **고급 붙여넣기** — 클립보드 텍스트를 대/소문자·공백 정리·여러 줄 합치기·Base64·URL 인/디코딩으로 변환. PowerToys의 "Advanced Paste"에 대응
- **창 배치** — **⌃⌥ 방향키**로 포커스 창을 화면 절반/최대화/가운데로 스냅, 메뉴에서 사분면까지. PowerToys의 "FancyZones"(키보드 버전)에 대응
...
1. **⌃⌥⌘O** 를 누르면 macOS 기본 십자선이 뜹니다.
2. 글자가 있는 화면 영역을 드래그하면, 그 안의 텍스트를 OCR해 **클립보드에 복사**합니다 (원하는 곳에 ⌘V). ESC로 취소.

> 캡처는 Apple 기본 `screencapture`에 위임합니다. 최초 1회 **화면 기록(Screen Recording)** 권한을 물을 수 있어요. 인식된 글자가 없으면 경고음이 납니다.

### 창 배치 — ⌃⌥ 방향키

포커스한 창을 현재 화면의 사용가능 영역(메뉴바·Dock 제외) 안에서 스냅합니다.
...
메뉴바 → **창 배치** 에서 위/아래 절반과 네 개 사분면까지 선택할 수 있어요. (창 이동에 **손쉬운 사용** 권한 필요. 앱의 최소 창 크기가 크면 리사이즈가 제한될 수 있습니다.)

### 화면 색상 추출 — ⌃⌥⌘C

1. **⌃⌥⌘C** 를 누르면 macOS 기본 스포이드(확대경)가 뜹니다.
2. 원하는 픽셀을 클릭하면 그 색의 **HEX(#RRGGBB)가 클립보드에 복사**됩니다. ESC로 취소.
3. 메뉴바 → **최근 색상** 에서 이전에 뽑은 색을 다시 복사할 수 있어요 (색상 스와치 표시).
...
- **화면에서 텍스트 추출** — ⌃⌥⌘O와 동일 (영역 OCR → 클립보드)
- **고급 붙여넣기 (클립보드 변환)** — 대/소문자, 공백 다듬기, 여러 줄 → 한 줄, Base64/URL 인·디코딩. 선택하면 클립보드 내용이 변환된 텍스트로 바뀝니다 (이후 ⌘V). 변환 불가(잘못된 Base64 등)면 경고음
...
- **창 배치** — 왼쪽/오른쪽/위/아래 절반, 네 개 사분면, 최대화, 가운데. 4개는 ⌃⌥ 방향키와 동일
- **잠자기 방지** — 서브메뉴에서 끔 / 켜기(화면은 꺼질 수 있음) / 켜기(화면도 켜둠) 선택. 켜면 부모 항목에 체크. 앱 종료 시 자동 해제
