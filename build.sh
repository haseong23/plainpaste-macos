...
# 서명 — SIGN_ID 환경변수 지정 시 해당 인증서, 미지정이어도 'PlainPaste Dev' 인증서가
# 키체인에 있으면 자동 사용(재빌드해도 손쉬운 사용 권한 유지 — Tests/make_signing_cert.sh 참고).
# 인증서가 없으면 기존처럼 ad-hoc(배포/install.sh 경로 무영향).
if [[ -z "${SIGN_ID:-}" ]] && security find-identity -v -p codesigning 2>/dev/null | grep -q '"PlainPaste Dev"'; then
    SIGN_ID="PlainPaste Dev"
fi
