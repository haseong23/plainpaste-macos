...
- **원본 클립보드 복원** — 붙여넣기 후 원본(이미지·서식 텍스트)을 되돌려, 이어서 일반 ⌘V로 원본을 그대로 붙여넣을 수 있음. 그 사이 새로 ⌘C 하면 복원을 취소해 새 복사를 덮어쓰지 않음
- **가벼움** — 의존성 0, 단일 Swift 파일, 바이너리 수백 KB. Dock 아이콘 없이 메뉴바에만 상주
...
4. 붙여넣기가 전달된 뒤 원본(이미지·서식) 클립보드를 복원 — 그 사이 새로 복사(⌘C)한 게 있으면 복원을 취소해 새 복사를 덮어쓰지 않음. modifier 대기·OCR 중 새 복사가 감지되면 늦은 붙여넣기 자체를 취소
